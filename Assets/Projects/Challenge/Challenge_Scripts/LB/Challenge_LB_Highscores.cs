...
namespace Challenge
{
    public class Challenge_LB_Highscores : LeaderboardParent
    {
        [Header("Display")]
        ...
        private void Update()
        {
            ...
            //    SceneManager.LoadScene(Challenge_GeneralVariables.Instance.accueilScene);
            //}
        }
        ...
        void SubmitScore() // This function handle all the display of the leaderboard
        {
            ...
            tempScore.player_score = PlayerPrefs.GetInt("Challenge_FinalScore");
        ...
        void ShowPlayerScores()
        {
                ...
                for (int i = scoreIdx; i < scoreIdx + scoreToShow; i++)
                {
                    ...
                    if (i < highscores.Count)
                    {
                        if (i == newScoreIndex)
                        {
                            ...
                            newScore.GetComponent<Challenge_LB_Scores>().SetupScore((1 + i).ToString() + ".", highscores[i].player_name, highscores[i].player_score.ToString(""), newScoreColor);
                        }
                        else
                        {
                            newScore.GetComponent<Challenge_LB_Scores>().SetupScore((1 + i).ToString() + ".", highscores[i].player_name, highscores[i].player_score.ToString(""));
                        }
                    }
                    else
                    {
                        newScore.GetComponent<Challenge_LB_Scores>().SetupScore((1 + i).ToString() + ".", Localizer.Get("Unknown"), "0");
                    }
                }
            }
            else
            ...
            //PlayerPrefs.SetInt("Challenge_FinalScore", 0);
        }
        ...
        void SavePlayersScore(List<HighScore_Data> scoresData) // save player score in a custom file
        {
            if (bf == null)
            {
                ...
                FileStream stream = new FileStream(Application.persistentDataPath + "/Challenge_Scores.sav", FileMode.Create, FileAccess.ReadWrite);
                bf.Serialize(stream, scoresData);
                stream.Close();
            }
            else
            {
                FileStream stream = new FileStream(Application.persistentDataPath + "/Challenge_Scores.sav", FileMode.Open, FileAccess.ReadWrite);
                bf.Serialize(stream, scoresData);
                stream.Close();
            }
        }
        ...
        {
            string path = Application.persistentDataPath + "/Challenge_Scores.sav";
        ...
        {
            if (File.Exists(Application.persistentDataPath + "/Challenge_Scores.sav"))
            {
                File.Delete(Application.persistentDataPath + "/Challenge_Scores.sav");
                highscores = null;
                ShowPlayerScores();
            }
        }
